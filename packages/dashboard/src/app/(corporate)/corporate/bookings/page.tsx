    ...
    <div className="flex flex-col h-full">
      ...
      <header className="h-16 border-b border-[#E2E8F0] bg-white flex items-center justify-between px-6 lg:px-8 flex-shrink-0">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">{myBookings.length} total bookings</p>
        </div>
      ...
      <main className="flex-1 overflow-y-auto p-8">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-xs font-semibold transition-colors ${
                filter === tab.key
                  ...
                  : "bg-slate-50 text-slate-500 border border-[#E2E8F0] hover:bg-slate-100 hover:text-slate-900"
              }`}
            >
              {tab.label}
              <span className="ml-1.5 bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded text-[10px]">{tab.count}</span>
            </button>
          ))}
        </div>
        ...
        <div className="space-y-4">
          {filtered.map((booking) => (
            <div key={booking.id} className="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm p-6">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className="text-base font-bold text-slate-900">{booking.hotel}</h3>
                  ...
                  <div className="flex items-center gap-1.5 mb-3">
                    ...
                    <span className="text-sm text-slate-600">{booking.destination}</span>
                  ...
                  <div className="flex items-center gap-6 text-sm text-slate-500">
                    ...
                    <span className="font-mono text-xs text-slate-500">
                      <i className="fa-solid fa-hashtag text-slate-400 mr-1" />
                ...
                <div className="text-right ml-6">
                  <p className="text-2xl font-bold text-slate-900">&euro;{booking.rate}</p>
                  <p className="text-xs text-slate-500">/night</p>
                  <p className="text-sm text-slate-500 mt-1">Total: &euro;{booking.totalCost}</p>
                  <div className="flex items-center gap-1 mt-1">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <i key={i} className={`fa-solid fa-star text-[9px] ${i < Math.floor(booking.rating) ? "text-amber" : "text-slate-200"}`} />
                    ))}
                    <span className="text-xs text-slate-500 ml-1">{booking.rating}</span>
                  </div>
                </div>
              </div>
