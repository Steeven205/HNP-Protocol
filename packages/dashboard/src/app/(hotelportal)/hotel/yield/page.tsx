  ...
  return (
    <>
      ...
      <header className="h-20 border-b border-white/10 glass-panel flex items-center justify-between px-6 flex-shrink-0">
        <div>
          <h1 className="font-display text-2xl font-bold text-white">AI-Powered Yield Configuration</h1>
          <p className="text-sm text-slate-400 mt-0.5">Natural language strategy to automated rules</p>
        </div>
      ...
      <main className="flex-1 overflow-y-auto p-6 space-y-6">
        {/* Property Selector */}
        <div className="glass-panel rounded-xl p-4 flex items-center gap-4 animate-fade-up">
          <label className="text-sm text-slate-400">Property</label>
          <select className="form-input-glass rounded-lg py-2 px-3 text-sm flex-1 max-w-md">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-fade-up delay-100">
          ...
          <div className="glass-panel rounded-2xl p-6">
            ...
            <textarea
              className="form-input-glass w-full rounded-lg p-4 text-sm h-32 resize-none"
              ...
              {[
                ...
                <div key={i} className="glass-card rounded-xl p-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium text-white">{g.rule}</span>
                    <span className="badge badge-emerald text-[10px]">{g.source}</span>
                  </div>
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div>
                      <span className="text-slate-400">Condition:</span>
                      <p className="text-slate-300 font-mono mt-0.5">{g.condition}</p>
                    </div>
                    <div>
            ...
            <table className="data-table w-full text-left">
              ...
              <tbody>
                {cfg.volumeDiscounts.map((v) => (
                  <tr key={v.threshold}>
                    <td>
                      <span className="text-white font-mono">{v.threshold}+</span>
                      <span className="text-slate-400 ml-1 text-xs">nights</span>
                    </td>
                    <td>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {(Object.entries(cfg.autoInclusions) as [string, boolean][]).map(([key, enabled]) => {
              ...
              return (
                <label
                  key={key}
                  className={`glass-card rounded-xl p-4 flex flex-col items-center gap-2 cursor-pointer transition-all ${
                    enabled ? "border-emerald/30 bg-emerald/5" : ""
                  }`}
                >
                  <i className={`fa-solid ${info.icon} text-xl ${enabled ? "text-emerald" : "text-slate-500"}`} />
                  <span className={`text-sm ${enabled ? "text-white" : "text-slate-400"}`}>
                    {info.label}
                  </span>
                  <input
                    type="checkbox"
