    ...
    <>
      ...
      <header className="h-20 border-b border-white/10 glass-panel flex items-center justify-between px-6 flex-shrink-0">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">Overview &amp; Performance</p>
        </div>
      ...
      <main className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 animate-fade-up">
          {kpis.map((kpi, i) => (
            <div
              key={kpi.label}
              className={`glass-card rounded-2xl p-5 ${kpi.warning && !kpi.trendUp ? "border-t-4 border-t-amber" : ""} delay-${(i + 1) * 100}`}
            >
              ...
              {kpi.showCircle && (
                <div className="mt-3 flex items-center gap-2">
                  ...
                  <span className="text-xs text-slate-400">of 100% capacity</span>
                </div>
              )}
            </div>
          ))}
                    ...
                    <tr key={p.id}>
                      ...
                      <td>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-white">{p.occupancy}%</span>
                        </div>
                      </td>
                      <td className="text-white font-mono">{p.adr}</td>
                      <td className="text-white font-mono">{p.revpar}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="p-4 space-y-3">
              {hotelNegotiations.slice(0, 4).map((n) => (
                <a
                  ...
                  className="block glass-card rounded-xl p-4 hover:border-emerald/30 transition-all"
                >
                  ...
                  <div className="text-sm text-white font-medium">{n.corporate}</div>
