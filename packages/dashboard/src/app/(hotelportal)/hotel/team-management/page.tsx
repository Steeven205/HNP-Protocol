...
const teamMembers = [
  { name: "Marie Dupont", email: "marie.dupont@lemaraisgroup.com", role: "Admin", roleIcon: "fa-shield-halved", roleBg: "bg-emerald-100 text-emerald-700 border-emerald-200", lastActive: "2h ago", avatar: "MD" },
  { name: "Jean Martin", email: "jean.martin@lemaraisgroup.com", role: "Manager", roleIcon: "fa-user-tie", roleBg: "bg-amber-100 text-amber-700 border-amber-200", lastActive: "1d ago", avatar: "JM" },
  { name: "Sophie Bernard", email: "sophie.bernard@lemaraisgroup.com", role: "Manager", roleIcon: "fa-user-tie", roleBg: "bg-amber-100 text-amber-700 border-amber-200", lastActive: "3h ago", avatar: "SB" },
  { name: "Thomas Roux", email: "thomas.roux@lemaraisgroup.com", role: "Viewer", roleIcon: "fa-eye", roleBg: "bg-slate-100 text-slate-600 border-slate-200", lastActive: "2d ago", avatar: "TR" },
  { name: "Claire Moreau", email: "claire.moreau@lemaraisgroup.com", role: "Viewer", roleIcon: "fa-eye", roleBg: "bg-slate-100 text-slate-600 border-slate-200", lastActive: "5h ago", avatar: "CM" },
];

const permissionsMatrix = [
  { name: "View Dashboard", admin: "check", manager: "check", viewer: "check" },
  { name: "View Negotiations", admin: "check", manager: "check", viewer: "check" },
  { name: "Override Negotiations", admin: "check", manager: "check", viewer: "x" },
  { name: "Edit Yield Configuration", admin: "check", manager: "check", viewer: "x" },
  { name: "Manage Hotels", admin: "check", manager: "limited", viewer: "x" },
  { name: "View Analytics", admin: "check", manager: "check", viewer: "limited" },
  { name: "Export Reports", admin: "check", manager: "check", viewer: "x" },
  { name: "Manage Integrations", admin: "check", manager: "x", viewer: "x" },
  { name: "Manage Team Members", admin: "check", manager: "x", viewer: "x" },
  { name: "View Audit Trail", admin: "check", manager: "check", viewer: "x" },
  { name: "Manage Billing", admin: "check", manager: "x", viewer: "x" },
];
      ...
      <div className="bg-white rounded-xl border border-[#E5E7EB] shadow-sm overflow-hidden">
        <div className="p-6 border-b border-[#E5E7EB] bg-[#F9FAFB] flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
          ...
          <button onClick={() => setShowInvite(true)} className="bg-emerald-500 hover:bg-emerald-600 text-white px-6 py-2.5 rounded-lg text-sm font-medium flex items-center gap-2">
            <i className="fa-solid fa-plus" /> Invite Team Member
          </button>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-[#E5E7EB]">
                ...
                <th className="py-4 px-4 text-center">
                  <div className="flex flex-col items-center gap-1">
                    <i className="fa-solid fa-eye text-lg text-[#6B7280]" />
                    <span className="text-xs font-semibold text-[#6B7280] uppercase">Viewer</span>
                  </div>
                </th>
              </tr>
            </thead>
            <tbody>
              {permissionsMatrix.map((p) => (
                <tr key={p.name} className="border-b border-[#F3F4F6] hover:bg-[#F9FAFB] transition-colors">
                  ...
                  <td className="py-3 px-4 text-center"><PermIcon type={p.viewer} /></td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="bg-white rounded-xl border border-[#E5E7EB] shadow-xl max-w-lg w-full mx-4" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-[#E5E7EB]">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-sm text-[#6B7280]">Send an invitation to join your team</p>
                </div>
            ...
            <div className="p-6 space-y-5">
              <div>
                ...
                <input type="email" placeholder="colleague@lemaraisgroup.com" className="w-full border border-[#E5E7EB] rounded-lg py-3 px-4 text-sm text-[#111827] focus:outline-none focus:border-emerald-500" />
              </div>
              <div>
              ...
              <div>
                ...
                <textarea rows={4} placeholder="Add a personal message..." className="w-full border border-[#E5E7EB] rounded-lg py-3 px-4 text-sm text-[#111827] focus:outline-none focus:border-emerald-500 resize-none" />
              </div>
            </div>
