...
const agentStyles: Record<string, { bg: string; border: string; icon: string; label: string; iconBg: string }> = {
  ...
  hotel: {
    ...
    icon: "fa-hotel",
    label: "Hotel Agent",
    iconBg: "bg-emerald-100 text-emerald",
  },
  system: {
    bg: "bg-slate-50",
    ...
    <>
      ...
      <header className="h-16 border-b border-[#E2E8F0] bg-white flex items-center justify-between px-6 lg:px-8 flex-shrink-0">
        <div className="flex items-center gap-3">
          <Link href="/hotel/negotiations" className="text-slate-500 hover:text-slate-900 transition-colors">
            <i className="fa-solid fa-arrow-left mr-2" />
            Negotiations
          </Link>
          <span className="text-slate-300">/</span>
          <span className="font-mono text-emerald font-medium">{neg.id}</span>
          <h1 className="font-display text-xl font-bold text-slate-900 ml-2">Negotiation Detail</h1>
        </div>
      ...
      <main className="flex-1 overflow-y-auto p-6 space-y-6">
        {/* Summary Card */}
        <div className="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm p-6 animate-fade-up">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-3">
                  <h2 className="font-display text-xl font-bold text-slate-900">{neg.id}</h2>
            ...
            <div className="flex items-center gap-6 text-sm">
              ...
              <div className="text-center">
                ...
                <p className="text-slate-900 font-mono">{neg.nights}</p>
              </div>
              <div className="text-center">
                <p className="text-xs text-slate-500">Room</p>
                <p className="text-slate-900 font-mono">{neg.roomType}</p>
              </div>
              {/* Countdown Timer */}
              <div className="bg-white rounded-lg border border-[#E2E8F0] shadow-sm px-4 py-2 border-emerald/30">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-[#E2E8F0] shadow-sm overflow-hidden animate-fade-up delay-100">
            ...
            <div className="p-6 space-y-4 max-h-[500px] overflow-y-auto">
              ...
              {/* Typing indicator */}
              <div className="flex gap-4 bg-amber-50 border-l-4 border-l-amber rounded-r-xl p-4">
                <div className="w-10 h-10 rounded-full bg-amber-100 text-amber flex items-center justify-center flex-shrink-0">
                ...
                <div className="flex items-center gap-1 py-2">
                  ...
                  <span className="w-2 h-2 bg-amber rounded-full typing-dot" />
                  <span className="w-2 h-2 bg-amber rounded-full typing-dot" />
                  <span className="text-xs text-slate-500 ml-2">Corporate Agent is reviewing...</span>
                </div>
              </div>
            </div>
          </div>
                      ...
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span
                          className={`text-sm font-mono font-bold ${
                            isLast ? "text-emerald" : "text-slate-900"
                          }`}
                        >
                          {r.rate}
                        </span>
                      ...
                      <div className="w-full h-2 rounded-full bg-slate-100 overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all duration-500 ${
                            isLast ? "bg-emerald" : "bg-slate-300"
                          }`}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div key={check.label} className="flex items-center gap-2">
                    ...
                    <span
                      className={`text-sm ${
                        check.ok ? "text-slate-600" : "text-amber"
                      }`}
                    >
                      {check.label}
                    </span>
